...
func initDB(cfg *Config) (*DB, error) {
	// 构建DSN
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%d)/richx?charset=utf8mb4&parseTime=True&loc=Local",
...
func initKafka(cfg *Config) (*KafkaProducer, error) {
	// 配置Kafka生产者
	config := sarama.NewConfig()
	config.Producer.RequiredAcks = sarama.WaitForAll
	config.Producer.Retry.Max = kafkaMaxRetry
	...
	if cfg.Kafka.SASL.Username != "" {
		...
		config.Net.TLS.Enable = false // 测试环境可能不需要TLS
	}

	// 连接到Kafka集群
	producer, err := sarama.NewSyncProducer([]string{cfg.Kafka.Brokers}, config)
	if err != nil {
		// 如果连接失败，使用Mock生产者
...
func main() {
	...
	if kafkaProducer.enabled && kafkaProducer.producer != nil {
		logger.Printf("Kafka连接成功，使用主题: %s", kafkaProducer.topic)
	} else {
		logger.Printf("Kafka未启用，将采用同步处理方式")
	}
